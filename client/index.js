...
const init = async () => {
	...
	// console.log(tonweb.utils.newSeed())

	const seedA = TonWeb.utils.base64ToBytes('SX2sNmZ9N70oAqEClONE0p7uMTd3bbvpmp0URbkyXoo='); // A's private (secret) key
	const keyPairA = tonweb.utils.keyPairFromSeed(seedA); // Obtain key pair (public key and private key)

	const seedB = TonWeb.utils.base64ToBytes('VVdh9U35EHUE5uSFBfpiI6IjKiI9s5mSnBWI4l9EuFc='); // B's private (secret) key
	...
	// console.log('checking channel address...')
	// await wait(5000);
...
return;
	...
	console.log(await channelA.getChannelState());
	let dd = await channelA.getData();
	console.log('balanceA = ', dd.balanceA.toString())
	console.log('balanceB = ', dd.balanceB.toString())
	...
	const channelState3 = {
		balanceA: toNano('0.25'),
		balanceB: toNano('0.25'),
		seqnoA: new BN(2),
		seqnoB: new BN(1)
	};
	...
	let res = await fromWalletA.close({
		...channelState3,
